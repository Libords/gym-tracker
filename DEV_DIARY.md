...
`2026-05-28 — VISIONS 16.12 custom exercises (vytvoření vlastního cviku) — migrace 20260528_custom_exercises.sql (ENABLE RLS na exercises + policies: SELECT is_custom=false OR created_by=auth.uid(), INSERT/UPDATE/DELETE jen vlastní custom; GRANT INSERT/UPDATE/DELETE pro authenticated; bezpečné — všech 873 seed cviků má is_custom=false takže SELECT je nadále vidí). useExercises rozšířen: fetchExercises jako useCallback, createExercise (INSERT is_custom=true + created_by=user.id, equipment uložen jako chip hodnota kterou mapEquipmentToChip zpětně rozpozná), refetch. Nová komponenta CreateExerciseModal (název + partie chip z bodyParts + vybavení chip z EQUIPMENT_CHIPS, validace, prefill názvu ze search). Wire do obou pickerů (workouts/[id] + templates/[id]): tlačítko "+ Vlastní cvik" v result row + "vlastní" badge u custom cviků. Po vytvoření: workout detail → setSelectedExercise (přejde na zadání série), template → handleAddExercise (přidá do šablony). ZBÝVÁ uživatel: aplikovat migraci 20260528_custom_exercises.sql. tsc pass — OK`

`2026-05-28 — Custom exercise modal fix: tlačítko "+ Vlastní cvik" nic nedělalo — CreateExerciseModal byl sourozenec už otevřeného picker-modalu (iOS nezobrazí 2. modal nad 1.). Fix: vnořen dovnitř picker-modalu v obou screenech. Ověřeno uživatelem = funguje. commit ebebd85 — OK`

`2026-05-28 — VISIONS 16.11 první milestone: "Previous" reference (Strong-style). Nová exportovaná funkce fetchLastExercisePerformance(userId, exerciseId, excludeWorkoutId) v useWorkouts — query workout_sets s !inner join na workouts (finished, user, ne aktuální workout), client-side vybere sety z nejnovějšího předchozího workoutu. workouts/[id].tsx: při výběru cviku v add-set flow se načte poslední výkonnost, zobrazí karta "Naposledy (DD.MM.): 80×8 80×7" a předvyplní reps/weight ze série odpovídající pořadí dnešní série (doneCount index, fallback poslední). Bez DB změny (jen čtení). Vejde se do stávajícího UI bez přepisu obrazovky. tsc pass — OK`

`2026-05-28 — VISIONS 16.11 milestone: 1RM odhad v historii tréninku. Nový lib src/lib/oneRepMax.ts (estimate1RM přes Epley: w×(1+reps/30), null pro sety bez váhy/reps). workouts/history/[id].tsx: groupByExercise počítá best_1rm per cvik, zobrazeno v hlavičce cviku (Max • Objem • 1RM) + per-set "1RM Xkg" vpravo. Respektuje preferred_unit přes formatWeight. Bez DB změny. tsc pass — OK`

`2026-05-28 — VISIONS 16.11 milestone: PR odznaky (osobní rekordy) v historii. Nová funkce fetchExercisePriorBests(userId, exerciseIds, beforeStartedAt) v useWorkouts — all-time maxWeight/maxReps/max1RM per cvik z finished workoutů PŘED daným tréninkem (jedna query, !inner join + .lt na started_at). history/[id].tsx: prFlags useMemo počítá per-set PR (WEIGHT/REPS/1RM) proti prior best + running best v rámci session; zobrazeno jako 🏆 odznaky u série (CS labely Váha/Opak./1RM). První výkon u cviku = vše PR (správně, jako Strong). Bez DB změny (jen čtení). tsc pass — OK`

`2026-05-28 — VISIONS 16.11 milestone: warmup série (is_warmup). Migrace 20260528_warmup_sets.sql (ADD workout_sets.is_warmup boolean DEFAULT false). WorkoutSet typ + is_warmup. workouts/[id]: warmup toggle (oranžový checkbox "Zahřívací série") v add-set kroku, předáno do addSet, reset po přidání; W badge (oranžový) u warmup série v aktivním seznamu. Warmupy vyloučeny ze stats všude: fetchExercisePriorBests + fetchLastExercisePerformance (.eq is_warmup false), doneCount prefill počítá jen working sety, history groupByExercise (max/objem/1RM jen working), history stats total volume, prFlags přeskakuje warmupy. History detail: warmup série má "W" místo čísla, bez 1RM/PR. ZBÝVÁ uživatel: aplikovat migraci 20260528_warmup_sets.sql. tsc pass — OK`
