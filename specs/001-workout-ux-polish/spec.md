...
- **FR-003**: Systém MUSÍ umožnit uživateli spustit nový workout ze šablony jediným potvrzením; nový workout obsahuje cviky v daném pořadí a u každého cviku target počet sérií předvyplněných hodnotami target reps a target weight ze šablony. Nový workout MUSÍ převzít `name` ze šablony (uživatel může v detailu workoutu přejmenovat). Uživatel může předvyplněné hodnoty kdykoli při běhu workoutu přepsat.
