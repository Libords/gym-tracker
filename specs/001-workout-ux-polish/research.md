...
**Permissions handshake**: per FR-017 — při prvním spuštění rest timeru zavolat `Notifications.getPermissionsAsync()`. Pokud `granted === false` a `canAskAgain === true`, zavolat `requestPermissionsAsync()`. Pokud `denied`, zobrazit Alert s tlačítkem `Linking.openSettings()`. Permission výsledek lze uložit do AsyncStorage flagu `restTimer.permissionDialogShown` aby se dialog neopakoval v rámci session (resp. dokud uživatel nezmění OS settings).
